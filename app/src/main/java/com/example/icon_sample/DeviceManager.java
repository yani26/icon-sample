...
public class DeviceManager implements Runnable, client.DeviceClientEvent  {
    ...
    private client.UDPClient udpClient;
    ...
    public void connect(boolean success, String err) {
        ...
        if (err != null && !success) {
            if (err.contains("established")) { // err does not contain established.
                Log.i(TAG," Do something ....");
                connected = true;
            }
        }

        if ( ! err.contains("established") && ! err.contains("started")) {

            EventBus.getDefault().post(new DeviceConnectEvent(success));
        }
    }
    ...
    public void qrCodeResult(String result) {
        ...
        EventBus.getDefault().post(new DeviceQRCodeResultEvent(result));
